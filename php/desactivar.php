...
if (empty($_POST['csrf_token']) || !SecurityHelper::validateCSRFToken($_POST['csrf_token'])) {
    SecurityHelper::logSecurityEvent('CSRF token validation failed on deactivate user', 'warning');
    header('refresh:3;url=usuario_listado.php');
...
if ($resultado) {
    SecurityHelper::logSecurityEvent('User deactivated successfully: ' . $id, 'info');
    header('refresh:3;url=usuario_listado.php');
    echo '<p>Usuario desactivado exitosamente.</p>';
} else {
    SecurityHelper::logSecurityEvent('Failed deactivate action for ID: ' . $id, 'error');
    header('refresh:3;url=usuario_listado.php');
    echo '<p>No se pudo desactivar el usuario.</p>';
}
